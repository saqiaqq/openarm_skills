...
ros2 action send_goal /openarm/pick_place openarm_skills/action/PickPlace "{cmd_id: 'test-pp-1e', arm: 'left', pose_source: 'upper_computer',
  ...
  grasp_pose: {position: {x: -0.25, y: 0.08, z: 0.28},
              orientation: {x: -0.7071, y: 0.0, z: 0.7071, w: 0.0}},
  place_pose: {position: {x: 0.48, y: 0.18, z: 0.6},
              orientation: {x: 0.7071, y: 0.0, z: 0.7071, w: 0.0}},
  approach_offset_m: 0.05, retreat_offset_m: 0.05,
...
案例1f（右臂：身后抓取 → 身前放置，已仿真验证）：
  要点：
  - grasp 在身后（-X, y<0 为右臂侧），place 在身前（+X）；四元数用水平 palm 朝上，勿用 (1,0,0,0) 垂直朝下（身后点 IK 易失败）
  - target_radius：pick.approach 绕抓取点球体；transport/place.approach 不避障；place.retreat 绕放置点球体
  - place.z=0.68 可达；若需更稳可改为 z=0.52（见下方注释）
ros2 action send_goal /openarm/pick_place openarm_skills/action/PickPlace "{cmd_id: 'test-pp-1e', arm: 'right', pose_source: 'upper_computer',
  target_name: '', target_index: 0,
  grasp_pose: {position: {x: -0.25, y: -0.08, z: 0.28},
              orientation: {x: -0.7071, y: 0.0, z: 0.7071, w: 0.0}},
  place_pose: {position: {x: 0.48, y: -0.18, z: 0.6},
...
# --- 4. hand_water (递水：身后抓取 → 身前悬停，保持夹持) ---
: <<'HAND_WATER_NOTES'
...
API 调用示例（openarm_api JSON）：
  {"cmd_id":"uuid","cmd_type":"hand_water","arm":"left","params":{"speed_scale":0.10}}

Feedback 阶段：
  pick.* → transport → place.approach → done
HAND_WATER_NOTES
